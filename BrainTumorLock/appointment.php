...
function insertDataIntoDatabase($name, $email, $phone, $date, $message)
{
    global $conn;

    // Use prepared statement with parameterized query
    $stmt = $conn->prepare("INSERT INTO appointment_requests (name, email, phone, date,  message) VALUES (?, ?, ?, ?, ?)");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($data !== null && is_array($data) && !empty($data["name"]) && !empty($data["email"]) && !empty($data["phone"]) && !empty($data["date"]) && !empty($data["message"])) {
        ...
        insertDataIntoDatabase($data["name"], $data["email"], $data["phone"], $data["date"], $data["message"]);
    } else {
